...
NSString *const mk_lb_additionalInformation = @"mk_lb_additionalInformation";
NSString *const mk_lb_dataInformation = @"mk_lb_dataInformation";
NSString *const mk_lb_dataStatusLev = @"mk_lb_dataStatusLev";

@interface MKLBOperation ()
...
/**
 线程ID
 */
@property (nonatomic, assign)mk_lb_taskOperationID operationID;
...
/**
 线程结束时候的回调
 */
@property (nonatomic, copy)void (^completeBlock) (NSError *error, mk_lb_taskOperationID operationID, id returnData);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKLBTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKLBTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
...
- (void)startReceiveTimer{
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        if (weakSelf.timeout || weakSelf.receiveTimerCount >= 50) {
            //接受数据超时
    ...
    if (self.completeBlock) {
        if (self.needPartOfData) {
            ...
            NSDictionary *resultDic = @{
                                        mk_lb_additionalInformation:(self.additionalInformation ?: @{}),
                                        mk_lb_dataInformation:self.dataList,
                                        //对于有附加信息的，lev为2，对于普通不包含附加信息的，lev为1.
                                        mk_lb_dataStatusLev:(self.additionalInformation ? @"2" : @"1"),
                                        };
            if (self.completeBlock) {
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_lb_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_lb_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    NSString *numString = returnData[mk_lb_communicationDataNum];
